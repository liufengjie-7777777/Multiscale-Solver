...
for k3=1:n(3) %Parameter Values
    for k1=1:n(1) %lz
        for k2=1:n(2) %Pin
            
            load([varName ' Simulation lzandPin\' Sim varName ...
                '(' num2str(k3) ')' '_Simulation' '(' num2str(n(2)*(k1-1)+k2) ').mat']);
            
            %Update material parameters that changed
            b.UpdateParameters(a);
            strLegend{k3} = [varName, ' = ', num2str(b.(varName)*unitDets{1}), unitDets{2} ];
            ...
            Do(k1,k2,k3) = b.ro*2e3; %um
            FT(k1,k2,k3) = b.FTCalc*1e3; %mN
...
if 0
    ...
    for i=1:size(Do,3)
        h(i,:) = plot3(xPin,ylz,Do(:,:,i),symVec{i});
        if i == 1
            hold on;
        end
    end
    ...
    figure();
    for i=1:size(FT,3)
        h(i,:) = plot3(xPin,ylz,FT(:,:,i),symVec{i});
        if i == 1
            hold on;
        end
    end
